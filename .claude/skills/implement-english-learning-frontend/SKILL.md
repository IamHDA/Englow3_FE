...
description: Implement, extend, refactor, or review the English Learning web frontend built with Next.js App Router and TypeScript, talking to an Apollo GraphQL BFF. Use for pages, layouts, reusable components, onboarding, exam taking, quizzes, speaking practice, progress dashboards, forms, GraphQL queries and mutations, loading and skeleton states, authentication-aware UI, Lucide and custom icons, and frontend tests. Trigger it even when the request sounds like a plain task ("add a page for exam results", "why does this flash on load", "where should this component go") and names none of the above, because the placement, loading, and browser-security rules still govern the answer.
---

# English Learning Frontend

Next.js App Router, TypeScript, React Hook Form with Zod, Apollo Client against a GraphQL BFF. The BFF is the only application backend the browser talks to.
...
2. Inspect `package.json`, the router in use, `tsconfig.json` path aliases, the styling system, the GraphQL codegen setup, the auth setup, a neighbouring feature, and its tests.
3. Reuse what is installed. Do not migrate between App Router and Pages Router, replace the styling system, or add a state or data-fetching library unless the task requires it.
...
3. **Every loading fallback is a `<Skeleton>` component.** Never a spinner, never a "Loading..." string, never an empty fragment.
